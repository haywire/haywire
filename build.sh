...
if [ $OS = $PLATFORM_WINDOWS ]; then
    ...
    echo "----------------------------------------"
    $GYP --depth=. -Goutput_dir=./builds/unix -Icommon.gypi -Dlibrary=static_library -Duv_library=static_library -Dtarget_arch=x86 --build=$CONFIGURATION -f make haywire.gyp
fi
